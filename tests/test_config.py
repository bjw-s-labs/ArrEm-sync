...
class TestConfig:
    ...
    def test_config_defaults(self):
        ...
        assert config.app_name == "arr-tagsync"
        assert config.log_level == "INFO"
        assert config.dry_run is False
        assert config.batch_size == 50
        assert config.emby_user_id is None
    ...
    def test_get_config_from_env(self, monkeypatch):
        ...
        monkeypatch.setenv("TAGSYNC_EMBY_URL", "http://test-emby:8096")
        monkeypatch.setenv("TAGSYNC_EMBY_API_KEY", "test_emby_key")
        monkeypatch.setenv("TAGSYNC_ARR_TYPE", "sonarr")
        monkeypatch.setenv("TAGSYNC_ARR_URL", "http://test-sonarr:8989")
        monkeypatch.setenv("TAGSYNC_ARR_API_KEY", "test_sonarr_key")
        monkeypatch.setenv("TAGSYNC_DRY_RUN", "true")
        monkeypatch.setenv("TAGSYNC_LOG_LEVEL", "DEBUG")
        monkeypatch.setenv("TAGSYNC_BATCH_SIZE", "25")
