...
from arr_tagsync.cli import cli, create_clients, setup_logging
from arr_tagsync.config import Config
...
class TestCLIUtilities:
    ...
    def test_create_clients(self):
        ...
        assert emby_client.user_id == "test_user_id"

    def test_create_clients_without_user_id(self):
        """Test client creation without user_id."""
        config = Config(
            arr_type="sonarr",
        ...
        # Check that emby client user_id is None
        assert emby_client.user_id is None

    @patch("arr_tagsync.cli.ArrClient")
    @patch("arr_tagsync.cli.EmbyClient")
    def test_create_clients_initialization_calls(self, mock_emby_client, mock_arr_client):
        ...
        mock_emby_client.assert_called_once_with(
            server_url="http://localhost:8096", api_key="test_emby_key", user_id="test_user_id"
        )
...
class TestCLICommands:
    ...
    @patch("arr_tagsync.cli.get_config")
    @patch("arr_tagsync.cli.create_clients")
    @patch("arr_tagsync.cli.TagSyncService")
